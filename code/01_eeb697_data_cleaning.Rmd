...
For my term project I will be exploring brown pelican movement data to answer the question, are there individual differences in time spent over land vs. time spent over water?
...
Data was acquired from various sources for this analysis. The brown pelican data was publically available on Movebank.org (Geary B, Walter ST, Leberg PL, Karubian J (2018) Condition-dependent foraging strategies in a coastal seabird: evidence for the rich get richer hypothesis. Behavioral Ecology. doi:10.1093/beheco/ary173). 
...
gom_shp_trans <- st_transform(gom_shp, crs = st_crs(pel_crs)) # NOTE: this shapefile is just for plotting purposess
...
gom_shp_trans_crop <- st_crop(gom_shp_trans, st_bbox(pel_dat_sf))


# GULF OF MEXICO DEPTH:
# match the gom depth shape data to the gom shape file
gom_depth_shp_trans <- st_transform(gom_depth_shp, crs = st_crs(pel_crs))

# check out the depth data by plotting it
plot(gom_depth_shp_trans)

# spatial join gulf of mexico depth to point data
gom_pel_join <- st_join(pel_dat_sf, gom_depth_shp_trans)
```
...
# pel_tracks_trans <- amt::transform_coords(pel_tracks, pel_crs) # why do we need this step? They're already in crs 4326
...
# plot all of the tracks together
ggplot() +
  geom_sf(data = )
