...
pel_tracks_unnested <- pel_tracks_steps %>% unnest(cols = steps)
...
# plot step lengths
pel_tracks_steps %>% 
  filter(!animal.id=="3511") %>% # I did need to remove it, it was throwin off the feng shui with a LOT more stationary points than any other animal
...
```{r}
# save tracks for analysis script
save(pel_tracks_steps, file = "data/output/tracks.RData")

# save gom_shp_trans_crop
save(gom_shp_trans_crop, file = "data/output/gulf_shp.RData")

# save tracks
write_rds(pel_tracks_steps, "pel_tracks_steps")
```
